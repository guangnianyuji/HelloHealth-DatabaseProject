  ...
  <el-scrollbar height="800px">
    ...
    <div>
      <el-card class="cardStyle" v-if="isLogin && !isAdministrator">
        <el-button class="coinButton" v-if="isLogin" type="text" @click="goToCoinDetail">
          我的杏仁币：
          {{ numOfCoin }}
        </el-button>
      </el-card>
  ...
  mounted() {
    ...
    axios.get('http://127.0.0.1:4523/m2/2961538-0-default/95495428')
        .then(response => {
          ...
          this.userInfo = responseData.map(userData => ({
          ...
          this.certification = responseData2.map(doctorData => ({
          ...
          this.administrator = responseData3.map(administratorData => ({
